...
The easiest way to install `aax-convert` is using [uv](https://github.com/astral-sh/uv):

```bash
# Install from the repository
uv tool install git+https://github.com/trucraft/aax-convert.git
...
```bash
pip install git+https://github.com/trucraft/aax-convert.git
```
...
```bash
# Basic usage
aax-convert -a YOUR_AUTHCODE input.aax

# Convert multiple files in parallel
aax-convert -a YOUR_AUTHCODE -p 4 *.aax

# Extract only cover art
aax-convert -a YOUR_AUTHCODE -i input.aax

# Convert to mono (smaller file size)
aax-convert -a YOUR_AUTHCODE -m input.aax

# Create M4B audiobook with embedded chapters (single file)
aax-convert -a YOUR_AUTHCODE -f m4b input.aax
...
options:
  -h, --help            show this help message to exit
...
- ✅ M4B creates single file with native chapter markers (no splitting)
